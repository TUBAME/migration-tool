...
<xsl:apply-templates select="." mode="title"/>====<xsl:apply-templates select="node()[not(self::title)]"/><xsl:value-of select="util:carriage-returns(1)"/>====
<xsl:value-of select="util:carriage-returns(2)"/>
</xsl:template>
...
<xsl:if test="mediaobject/imageobject[@condition='web']/imagedata/@fileref">
image::<xsl:value-of select="mediaobject/imageobject[@condition='web']/imagedata/@fileref"/>[]
</xsl:if>
...
programlisting/textobject/textdata require parent's figure tag  !!!
    
problem textdata :
---
textdata href=<xsl:value-of select="@fileref"/>
---
