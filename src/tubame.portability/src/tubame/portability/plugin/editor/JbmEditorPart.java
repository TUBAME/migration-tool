...
public class JbmEditorPart extends AbstractJbmEditorPart {
	 ...
	 * Logger
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(JbmEditorPart.class);
	/**
	 * Hearing column size
	 */
	...
	public boolean save(String fileName, TreeViewer treeViewer) {
		...
		try {
			JbmAccessFactory.getJbmWriteFacade().update(fileName, (List<JbmEditorMigrationRow>) treeViewer.getInput());
			WorkStatusView.out(false, null);
			LOGGER.debug(MessageUtil.INF_SAVED);
		} catch (JbmException e) {
			// Failed to save
			PluginUtil.viewErrorDialog(ResourceUtil.JBM_EDITOR,
					MessageUtil.ERR_SAVE + StringUtil.LINE_SEPARATOR + e.getMessage(), e);
		}
		return true;
	}
	...
	public void addColumn(Tree tree) {
		...
		super.createTreeColumn(tree, JbmEditorEnum.INDEX_NO.getName(), SWT.NULL, JbmEditorPart.NO_COLUMN_SIZE,
				selectionListener);

		super.createTreeColumn(tree, JbmEditorEnum.BIG_ITEM.getName(), SWT.NULL, JbmEditorPart.BIG_ITEM_COLUMN_SIZE,
				selectionListener);

		super.createTreeColumn(tree, JbmEditorEnum.MIDDLE_ITEM.getName(), SWT.NULL,
				JbmEditorPart.MIDDLE_ITEM_COLUMN_SIZE, selectionListener);

		super.createTreeColumn(tree, JbmEditorEnum.DIFFICULTY.getName(), SWT.NULL,
				JbmEditorPart.DIFFICULTY_COLUMN_SIZE, selectionListener);

		super.createTreeColumn(tree, JbmEditorEnum.HIT_NUM.getName(), SWT.RIGHT, JbmEditorPart.HIT_COLUMN_SIZE,
				selectionListener);

		super.createTreeColumn(tree, JbmEditorEnum.TARGET_FILE_PATH.getName(), SWT.NULL,
				JbmEditorPart.TARGET_FILE_PATH_COLUMN_SIZE, null);

		super.createTreeColumn(tree, JbmEditorEnum.ROW_NO.getName(), SWT.RIGHT, JbmEditorPart.ROW_COLUMN_SIZE, null);

		// Number of line (with sort)
		super.createTreeColumn(tree, JbmEditorEnum.LINE_NUM.getName(), SWT.NULL, JbmEditorPart.LINE_COLUMN_SIZE,
				selectionListener);

		// Line number basis
		super.createTreeColumn(tree, JbmEditorEnum.LINE_NUM_BASIS.getName(), SWT.SELECTED,
				JbmEditorPart.LINE_BASIS_COLUMN_SIZE, null);

		// Total line
		super.createTreeColumn(tree, JbmEditorEnum.TOTAL_LINE.getName(), SWT.NULL,
				JbmEditorPart.TOTAL_LINE_COLUMN_SIZE, selectionListener);

		super.createTreeColumn(tree, JbmEditorEnum.CHAPTER_NO.getName(), SWT.NULL,
				JbmEditorPart.CHAPTER_NO_COLUMN_SIZE, null);
		...
		// SWT.NULL, 0, null);
		super.createTreeColumn(tree, JbmEditorEnum.VISUAL_CONFIRM_STATSU_ITEM.getName(), SWT.NULL,
				JbmEditorPart.VISUAL_COLUMN_SIZE, selectionListener);
		super.createTreeColumn(tree, JbmEditorEnum.HIARING_STATUS.getName(), SWT.NULL,
				JbmEditorPart.HIARING_COLUMN_SIZE, selectionListener);
	 ...
	 */
	@Override
	public IDoubleClickListener getDoubleClickListener(MigrationEditorOperation editor) {
	...
	public void preInit() {
		try {
			...
			CheckListInformationFactory.getCheckListInformationFacade().initCheckListInformationReader();
