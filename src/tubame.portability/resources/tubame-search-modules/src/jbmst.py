        ...
        try :
            ...
            if search_str1 == "":
                for f in target_list:
                    jbmst_common.print_csv3(no,f,priority,flag,chapter_no,check_Status)
            else:   
                search_input_list=[(t, search_str1,search_str2) for t in target_list]
                result_line_cnt_list = []

                if extension == "java" or extension == "c" or extension == "h" :
                    result_line_cnt_list = p.map(jbmst_search_java.wrapSearchByFile, search_input_list)
                elif extension == "jsp" :
                    result_line_cnt_list = p.map(jbmst_search_jsp.wrapSearchByFile, search_input_list)
                elif extension == "xml" :
                    result_line_cnt_list = p.map(jbmst_search_xml.wrapSearchByFile, search_input_list)
                elif extension == "properties" :
                    result_line_cnt_list = p.map(jbmst_search_properties.wrapSearchByFile, search_input_list)
                elif extension == "sh" or extension == "csh":
                    result_line_cnt_list = p.map(jbmst_search_sh.wrapSearchByFile, search_input_list)
                elif extension == "sql" or extension == "ddl":
                    result_line_cnt_list = p.map(jbmst_search_sql.wrapSearchByFile, search_input_list)
                else:
                    result_line_cnt_list = p.map(jbmst_search_default.wrapSearchByFile, search_input_list)

                # Output to a CSV also number 0 line search hits.
                for result_tuple in result_line_cnt_list:
                    if len(result_tuple[0]) != 0:
                        jbmst_common.print_csv(no,priority,flag,result_tuple[1],result_tuple[0],chapter_no,check_Status)
